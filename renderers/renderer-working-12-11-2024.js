...
function renderSeriesList(data, checklist) {
    const seriesListContainer = document.createElement('div');
    seriesListContainer.classList.add('series-list');

    // Detectar si es "Furniture" o "Fish" y seleccionar los datos adecuados
    const items = data.Furniture || data.Fish;
    if (!items) {
        console.error("Formato de JSON no válido o datos faltantes");
        return;
    }

    // Ordenar alfabéticamente las series antes de renderizarlas
    ...
    items.forEach(series => {
        ...
        // Aplicar estilo de completitud
        if (completionPercentage === 100) {
            seriesLink.classList.add("golden-button");
            seriesLink.style.background = "";
        } else {
...
function loadCategoryData(filePath) {
    try {
        const data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
        const checklist = document.getElementById('checklist');
        
        // Llamar a renderSeriesList con el JSON cargado
        renderSeriesList(data, checklist);
    } catch (error) {
...
function renderAllItems(data, checklist) {
    data.forEach(series => {
        ...
        headerRow.innerHTML = `
            <th>✔️</th>
            <th>Item</th>
            <th>${series.items[0].bells ? 'Price (Bells)' : 'Value'}</th>
            <th>Source</th>
        `;
        ...
        series.items.forEach(item => {
            ...
            priceCell.textContent = item.bells ? `${item.bells} bells` : (item.value || '-');
